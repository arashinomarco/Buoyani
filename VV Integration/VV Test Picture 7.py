...
class Particle:
    def __init__(self, color):
        self.color = color
        self.position = np.random.rand(2) * 500 - 4.5
...
def update_acceleration(particles):
    for p in particles:
        p.acceleration = np.random.rand(2) * 500 - 1
...
image_extent = [-image.shape[1] / 0.5, image.shape[1] / 0.5, -image.shape[0] / 0.5, image.shape[0] / 0.5]
fig, ax = plt.subplots()
